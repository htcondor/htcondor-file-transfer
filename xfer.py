...
class ManifestEntry(metaclass=abc.ABCMeta):
    ...
    def __repr__(self):
        return "{}({})".format(
            type(self).__name__,
            ", ".join("{} = {!r}".format(k, v) for k, v in self._info.items()),
        )
...
def post_transfer(
    ...
    if direction is TransferDirection.PULL and not only_verify:
        logging.info(
            "This is a %s, renaming scratch file %s -> %s",
            direction,
            flattened_name,
            local_name,
        )
...
def analyze(transfer_manifest_path: Path) -> None:
    ...
    if len(mismatched_filesizes) > 0:
        ...
        raise InconsistentManifest(
            "Local sizes of {} files did match anticipated sizes.".format(
                len(mismatched_filesizes)
            )
        )
